...
u8 KEY_Scan(u8 mode)
{
    ...
    if(key_up&&(WK_UP==1||Motor3_Tim==1||Motor3_Alm==1||Motor4_Tim==1||Motor4_Alm==1||Motor5_Tim==1||Motor5_Alm==1||Motor6_Tim==1||Motor6_Alm==1||Motor7_Tim==1||PCF8574_ReadBit(1)==1))  
    {
        delay_us(5);       //按键消抖
        key_up=0;  
		...
		else if(PCF8574_ReadBit(1)==1) return Motor7_Alm_PRES;     //7号电机故障报警			
    }
	else if(WK_UP==0&&Motor3_Tim==0&&Motor3_Alm==0&&Motor4_Tim==0&&Motor4_Alm==0&&Motor5_Tim==0&&Motor5_Alm==0&&Motor6_Tim==0&&Motor6_Alm==0&&Motor7_Tim==0&&PCF8574_ReadBit(1)==0)key_up=1; 
    return 0;   //无按键按下
}
