...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef *htim)
{
	...
	{
		__HAL_RCC_TIM2_CLK_ENABLE();            //使能TIM7时钟
		HAL_NVIC_SetPriority(TIM2_IRQn,0,2);    //设置中断优先级，抢占优先级0，子优先级1
		HAL_NVIC_EnableIRQ(TIM2_IRQn);          //开启TIM7中断   
	}	
}
...
void TIM2_Stop(void)
{
	  HAL_TIM_Base_Stop_IT(&TIM2_Handler);  //关闭定时器2和定时器2更新中断
}
